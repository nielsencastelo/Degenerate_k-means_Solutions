% epsilon-Greedy
function centros = epsilon_greedy(x,classe,k,centros,cluster,epsilon,gr,cluster_cost,ponto_mais_distante,cluster_length)
...
for g = 1:gr
    ...
    centros_heap = pop_heap(centros_heap,k-gr+g-1,cluster_cost);
    cg = cluster(g);
    centros(cg,:) = (1-epsilon)*centros(c,:) + epsilon*ponto_mais_distante(c,:);
    sum_c = zeros(1,d);
    sum_cg = zeros(1,d);
    n_c = 0;
    n_cg = 0;
    ...
    for i = 1:n
        if classe(i) == c
            di2 = sum((x(i,:)-centros(c,:)).^2);
            cluster_cost(c) = cluster_cost(c) + di2;
            if di2 > max_di2_c
                max_di2_c = di2;
                ponto_mais_distante(c,:) = x(i,:);
            end
        elseif classe(i) == cg
            di2 = sum((x(i,:)-centros(cg,:)).^2);
            cluster_cost(cg) = cluster_cost(cg) + di2;
            if di2 > max_di2_cg
                max_di2_cg = di2;
